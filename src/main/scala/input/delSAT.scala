...
/**
  ...
  *
  */
object delSAT extends APITests {
  ...
  */
  ...
  val thirdPartyLibs = "delSAT " + version +
  ...
  val helpText =
    """
     ...
     """ + (if (seedRngGlobalR == -1l) "[random number]" else seedRngGlobalR) +
      ...
      printed with -n alone). Observe that delSAT possibly finds different
      ...
      If delSAT runs out of memory, increase JVM heap using -Xmx and use
      -gc 1.
      -Xmx is also relevant for off-heap(!) available memory estimation
      (lower -Xmx: more frequent off-heap garbage collection).

      --solverarg "argname" "value" is used to specify less often required
      solver arguments.
      ...
      software used by delSAT (including copyrights and license information),
      then exits.
  ...
  val osWin = System.getProperty("os.name").toLowerCase().contains("win")
  ...
  val delSATGlobalLock = new ReentrantLock()
  ...
  def obtainInputFromText(fileNameOpt: Either[Option[String], String], mseInCommandLineOpt: Option[Boolean]): (InputData, Boolean,
    ...
    */
    ...
    inlineAllModelsCheckTokensOpt, inlineAllModelsMultilineCheckTokensOpt) = if (runBatchTests) {
      ...
      // see _KnowledgeBase_delSAT_internal.txt
  ...
  (SamplingResult, AspifOrDIMACSPlainParserResult) = {
    ...
    if (maxAssumedConsoleWidth > 0 && System.getProperty("os.name").toLowerCase().contains("win")) {
  ...
  def main(args: Array[String]): Unit = {
    ...
    if (debug) println("Command line arguments in delSAT: " + parsedCommandlineArgs)
    ...
    do { // batch processing loop
      ...
      if (writeRuntimeStatsToFile) {
        ...
        stats.writeEntry(key = "solver", value = "delSAT " + version, solverThreadNo = 0)
