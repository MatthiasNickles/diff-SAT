...
package object aspIOutils {
  ...
  def extractModels(lines: IndexedSeq[String]): IndexedSeq[Model] = {
    ...
    val modelsStrs0 = lines.zipWithIndex.filter { case (line: String, i: Int) => {
      ...
      sa || lt.startsWith("Answer:") || lt.contains("SATISFIABLE")

      // Note that with clasp, each answer can consist of multiple lines
    ...
    modelsStrs1.flatMap(rangeOfAnswer => {
      ...
      val x = rangeOfAnswer.collect(lines).filterNot(_.trim.isEmpty).map(l => {
