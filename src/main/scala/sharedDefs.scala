...
package object sharedDefs {
  ...
  var restartTriggerConfP: (AlternativesForThreads[Int], Seq[Int], Seq[Double]) = (AlternativesForThreads[Int](Map(0->3, 2->2, 8->2)) /*
    ...
    Seq(0d, /*for approach 1 (fixed):*/ 1d, /*for approach 2 (geom):*/ 1.02d/*1.1d*/ /*1.02d*/
  ...
  var restartFrequencyModifierFactorP: AlternativesForThreads[Float] = AlternativesForThreads[Float](Map(0 -> 1f, 5 -> 0.2f /*1f*/
    /*-0.98f*/))
  ...
  val localRestartsTriggerThreshFactor = 1
  ...
  /** Data structure for (non-probabilistic) branching heuristics ("free eli search"), for conventional branching (i.e., where
    ...
    * 12: Tiered (decision level-wise) linear search without absEli score consideration
    * 13: As 12 but with absEli score consideration
    * 14: A heap based on an dual-indexed set. absEli scores are considered.
    * 15: Uses a kind of radix sort where the absEli scores clustered (rounded) into bins. Search is linear over ordered bins.
    *
  ...
  var freeEliSearchApproachP: AlternativesForThreads[Int] = AlternativesForThreads[Int](
    Map(0 -> 15, 3 -> 12, 8 -> 11) /*,hanoi5,2dlx_cc_ex_bp_f_bug1_liveness: 12*/
  ...
  val evsids = false //true // TODO: make this absEliScoringApproach 3

  /** Literal scoring approach for regular (non-parameter atom) branching heuristics (such as LRB) - see source code for details */
  var absEliScoringApproachP: AlternativesForThreads[Int] = AlternativesForThreads(Map(0 -> 0, 2 -> 2, 5 -> 2)) // 0: EVSIDS (Minisat) or adaptVSIDS (Liang et al) but
  // using nogoods and with some minor modfifications. adaptVSIDS is itself based on EVSIDS (as in MiniSAT, PicoSAT and others)).
  ...
  val alphaInit = 0.4f // 0.4f  // for absEliScoringApproach 1 and 2

  val alphaTh = 0.06f //0.06f  // for absEliScoringApproach 1 and 2

  val alphaStep = 1e-6f //1e-6f // for absEliScoringApproach 1 and 2

  val updSc = 0.95f //0.95f  // for absEliScoringApproach 1 and 2
  ...
  val updateHeapInFreeElisPriorityQueueEvery = 1 // for freeEliSearchConfigs == 11 and absEliScoringApproach != 0
  ...
  var baseGlucoseStyleNogoodRemovalStrategy = 40000 //20000

  /** factor parameter for nogood deletion strategy nogoodRemovalThreshInit=0
    * for nogoodRemovalThreshInit=0. Lower value = nogood removal procedure called more frequently. No effect if reduceLearnedNogoodAtRestarts=true */
  var factorGlucoseStyleNogoodRemovalStrategy = 2850 //500
  ...
  /** Scoring approach of nogoods for learned nogood removal sweeps.
    ...
    * 5: n/a
    * 6: n/a
    * 7: inverse nogood size (i.e., preferably shorted nogoods are kept, irrespective of LBD)
    ...
    */
  var scoringForRemovalOfLearnedNogoodsR = 11 //0  // TODO: make seq.
  ...
  /** If x > 0, the probability of picking a branching literal randomly if no unassigned parameter literal could be found.
    ...
    * 0 is typically the best choice: even with 0, there are other means of randomization, see
  ...
  val rndmBranchProbR = 0f //0f // 1e-4f // -1f // -5000f
  ...
  //^ for some extReducibles approaches (see code) only. TODO: remove or say for which ones (e.g.,
  // impossible for extReducibles=1)
  ...
  and nogoods. Tries to remove variables and nogoods with these variables before actual solving starts.*/ (false /*on/off*/ ,
    ...
    #literals can still become larger than original total #literals even with 0d here*/ ,
    /*0.1f*/ 1f /*10f*/
    /*maximum product of literals with positive or negative occurrence of variable candidate for removal, in % of sqrt(all literals)*/ ,
  ...
  /** See source code */
  var abandonOrSwitchSlowThreads: Double = 0d // suggestion: try with 0d (off) first, then, e.g. 10d or 2d.
  ...
  val clusterAbsElis = false && freeEliSearchApproachP.alternatives.contains(7) // TODO: keep? Clustering elis seems mostly (?) not helpful if the
  ...
  val removeDuplicatesFromClarkNogoods = false
  ...
  var sortInitialNogoods = false && shuffleNogoods == 0 && !clusterNogoods
  ...
  /** See source code */
  val extReducibles: Int = 2 // Should normally be 2 (the other approaches are experimental and might be removed in the future).
  ...
  var initialPhaseMemo: Int = 0 // 0 = 0x00,  // TODO: make seq
  ...
  /** If > 0, add noise to use of phase memo when using the phase of a branching eli.
    * If x <= 0, no phase memo is used: positive polarity with probabiliy -x, negative otherwise.
    * Observe that with adaptNoisePhaseMemo, it can make sense to provide a value >= 1 here. */
  var noisePhaseMemoP = AlternativesForThreads[Float](Map(0 -> 0.001f, 1 -> 0f, 2 -> 0.2f, 5 -> 0.2f)) //-0.5f //1e-8f

  /** If != 0, phase memo noise (see noisePhaseMemoR) changes over time at each restart
    * using a heuristics:
    * 0: no change,
    * 1: decrease with number of conflicts,
    * 2: fluctuating noise with amplitude and period decreasing with number of conflicts (details see code),
    * 3: like 2, but using the number of restarts instead of number of conflicts */
  var adaptNoisePhaseMemo: Int = 2
  // TODO: make seq
  ...
  var useSLSinPhaseMemoRephasingP = AlternativesForThreads[Boolean](Map(0 -> false))

  /** If number of variables outside this interval never use SLS */
  var boundsOnNoOfAbsElisForSLSinrephasing = (50, 128000)
  ...
  var walkSATnoiseR: Float = 0.3f //0.567f // remark: optimal value for 3SAT might be 0.567 (TODO: make var. Add "auto" and recognize 3SAT)
  ...
  var copyPhasesFromBestPhasesInSLS: Boolean = true //true
  ...
  var minTempInSLS: Double = 0.1d //0.1d  // for WalkSAT this is only relevant if walkSATwithTempDecl is true
  ...
  var decayRateSLS: Float = -100f //-1000f //-1f //1e-7f

  /** true disallows rephasing (including SLS) to run in parallel with other SLS.
  ...
  var runRephasingExclusively: Boolean = true

  // end of SLS settings ------------------------

  /** Traversal direction in some of the places where variables are searched or ordered. Can make a big difference but the optimal value is unfortunately
    ...
    * 3 = order of first appearance in nogoods, 4 = reverse order of first appearance
    ...
    * deprecated (might be removed soon): 9 = by number of occurrences as pos or neg eli. 10 = as 9 but reverse order.
    ...
    * from this order, for, e.g., freeEliSearchConfigs == 11. Also see absElisScoringPreservationFactorFromInit.
    */
  var initAbsElisArrangementP: AlternativesForThreads[Int] = AlternativesForThreads[Int](Map(0 -> 0))
  ...
  /** Higher values = the initial arrangement of absElis (see initAbsElisArrangementP) are
    ...
    * Very high values together with neverRescaleAbsEliScores=true effectively make absEliScoringApproachP meaningless. TODO: make var, seq
    * Negative values reverse the absEli score impact of the order defined by initAbsElisArrangement. */
  var absElisScoringPreservationFactorFromInit: Float = 0.01f // Float.MaxValue
  ...
  val omitSingletonBlits: Boolean = true // must leave at true!
